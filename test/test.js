const { getSchema, serialize, hydrate, deserialize } = require("../src/index");
const fs = require("fs");
...
function _getComplexPullRequest() {
  const URI = {
    match: /^https?:\/\//,
  ...
  const Milestone = {
    schema: getSchema(pullRequest.milestone, {
      schemas: { User, URI, DateTime },
    }),
    description: "A GitHub Milestone Object",
  };
  const Repository = {
    schema: getSchema(pullRequest.base.repo, {
      schemas: { User, URI, DateTime },
    }),
...
describe("deserialize", () => {
  it("Translates jsdoc comments into schemas", () => {
    ...
    const { PullRequest } = _getComplexPullRequest();
